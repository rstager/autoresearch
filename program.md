...
6. **Create git worktrees**: One worktree per GPU, used for parallel experiments:
   ```bash
...
3. **Spawn N sub-agents in parallel**: Use the Agent tool with `run_in_background=True` for each GPU. Pass each sub-agent:
...
6. **If improvement found**:
   - `git cherry-pick <winning_commit>` onto the main branch checkout
   - Mark winning experiment as `keep`, all others as `discard`
...
7. Run the experiment:
   ...
   ```
   Set a 10-minute wall-clock timeout. Kill and report crash if exceeded.
8. Check results:
   ```bash
   grep "^val_bpb:\|^peak_vram_mb:" run.log
   ```
